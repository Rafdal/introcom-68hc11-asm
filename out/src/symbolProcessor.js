...
class SymbolProcessor {
     ...
     */
    async provideSymbols(fileFilter, query, token) {
        const symbols = {};
        const searched = [];
        const output = [];
        if (fileFilter) {
        ...
        const alreadyProcessedDeclarations = [];
        Object.values(symbols).reverse().forEach(symbol => {
            if (!alreadyProcessedDeclarations.includes(symbol.declaration) &&
                ...
                output.push(new vscode.SymbolInformation(symbol.declaration, symbol.kind, '', symbol.location));
